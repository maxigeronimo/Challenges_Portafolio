...
const listarUnProduto = (id) => {
 return fetch(`http://localhost:3000/comentario/${id}`).then((resposta) => {
...
const creaProdutos = (name, imageUrl, price, description) => {
 return fetch(`http://localhost:3000/comentario`, {
   method: "POST",
   headers: {
...
const alteraProduto = async (id, name, price, description) => {
 return fetch(`http://localhost:3000/comentario/${id}`, {
   method: "PATCH",
   headers: {
     "Content-Type": "application/json",
   },
...
const deleteProducto = async (id) => {
 return await fetch(`http://localhost:3000/comentario/${id}`, {
   method: "DELETE",
   headers: {
     "Content-Type": "application/json",
   },
